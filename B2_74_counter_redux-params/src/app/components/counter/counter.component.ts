...
export class CounterComponent {
  ...
  increment() : void {
    //this.counter++;
    this.store.dispatch(increment({add : 3}));         //en Item.action.ts se definió la accion increment y en el items.reducer.ts se define el counter reducer. En el items.reducer.ts está el estado inicial del contador y con el increment se suma 1 al estado.
